...
########################################
ctrl_v_lanm = fread("/data/san/data1/users/david/intelligence_database/ctrl_v_LanM_vs_Pks.tsv")
...
create_contingency_table <- function(df, DUF95_df, control_name) {
  ...
  circ_v_control_df <- rbind(LanM_table, df) %>% 
    distinct() 
...
fwrite(tablex, 
  "/data/san/data0/users/david/intelligence/tables/lanthipeptide_pfam_table.tsv")
...
cowplot_x1 = cowplot::plot_grid(vplot1, vplot2, labels = c("(a)", "(b)"), ncol = 2,label_size = 10)
